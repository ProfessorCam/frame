...
def build_pipeline(fmt, node_id, width, height, framerate, output_path,
                   tmp_path=None, have=_have_element):
    """Build the GStreamer pipeline string and the final target/output paths.

    Pure (no GStreamer state touched) so it can be unit-tested.  ``have`` is a
    predicate telling whether a named element is available – injectable for
    tests.

    Returns ``(pipe_str, target_path, output_path)`` where ``target`` is what
    GStreamer writes to (an intermediate .mkv for GIF) and ``output_path`` is
    the user-facing file (possibly rewritten, e.g. mp4→webm without x264).
    """
    fps = int(framerate) if framerate else 30

    if fmt == 'gif':
        # Near-lossless intermediate so GIF conversion starts from clean source
        target = tmp_path or (output_path + '.tmp.mkv')
        enc = ('x264enc qp-min=0 qp-max=10 speed-preset=ultrafast '
               'tune=zerolatency ! matroskamux') if have('x264enc') else (
               'vp8enc min-quantizer=0 max-quantizer=4 cpu-used=4 '
               'end-usage=vbr target-bitrate=50000000 ! webmmux')
    elif fmt == 'mp4':
        target = output_path
        if have('x264enc'):
            h264p = 'h264parse ! ' if have('h264parse') else ''
            enc = f'x264enc tune=zerolatency speed-preset=superfast ! {h264p}mp4mux'
        else:
            enc = 'vp8enc deadline=1 cpu-used=16 ! webmmux'
            target = output_path.replace('.mp4', '.webm')
            output_path = target
    elif fmt == 'webm':
        target = output_path
        if have('vp9enc'):
            enc = ('vp9enc min-quantizer=0 max-quantizer=20 cpu-used=4 '
                   'end-usage=vbr target-bitrate=20000000 threads=4 ! webmmux')
        else:
            enc = ('vp8enc min-quantizer=0 max-quantizer=10 cpu-used=4 '
                   'end-usage=vbr target-bitrate=20000000 ! webmmux')
    else:
        target = output_path
        enc = ('vp8enc min-quantizer=0 max-quantizer=10 cpu-used=4 '
               'end-usage=vbr target-bitrate=20000000 ! webmmux')
    ...
    if fmt == 'mp4':
        rawcaps = (f'video/x-raw,framerate={fps}/1,format=I420,'
                   f'width={width},height={height}')
    else:
        rawcaps = f'video/x-raw,framerate={fps}/1'

    # A valve lets us pause by dropping frames while the (live) pipeline keeps
    # running – toggling pipeline state on a live pipewiresrc breaks EOS/finalize.
    pipe_str = (
        f'pipewiresrc path={node_id} do-timestamp=true ! '
        f'valve name=wpvalve drop=false ! '
        f'videoconvert ! videoscale ! videorate ! '
        f'{rawcaps} ! '
        f'{enc} ! filesink location="{target}"'
    )
    return pipe_str, target, output_path
...
class Recorder:
    ...
    def start(self, x, y, width, height, output_path, fmt='gif',
              framerate=30, cursor=True, callbacks=None):
    ...
    def stop(self):
        ...
        self._paused = False
        if self._pipeline:
            # Re-open the valve so any final buffers flush, then EOS to finalize.
            self._set_valve(False)
            self._pipeline.send_event(Gst.Event.new_eos())

    def pause(self):
        """Pause frame capture (drop frames) without finalizing the file."""
        if self._pipeline and self._recording and not self._paused:
            if self._set_valve(True):
                self._paused = True
                return True
        return False
    ...
    def resume(self):
        ...
        if self._pipeline and self._recording and self._paused:
            if self._set_valve(False):
                self._paused = False
                return True
        return False

    def _set_valve(self, drop):
        """Toggle the pipeline valve's drop property. Returns True on success."""
        if not self._pipeline:
            return False
        valve = self._pipeline.get_by_name('wpvalve')
        if valve is None:
            return False
    ...
    def _create_session(self, x, y, w, h):
        ...
        # cursor-mode: 1 = embedded in the stream, 0 = hidden
        props = {'cursor-mode': GLib.Variant('u', 1 if self._cursor else 0)}
    ...
    def _start_pipeline(self):
        pipe_str, target, output_path = build_pipeline(
            self._format, self._node_id, self._area_w, self._area_h,
            self._framerate, self._output_path,
        )
        if self._format == 'gif':
            self._tmp_path = target
        self._output_path = output_path
